...
- **EWMA has a cold-start blind spot.** A backend that just recovered — or was just added — has no latency samples, so its average reads as `0`, which the least-response-time strategy reads as "fastest backend available" and floods it before a single real measurement comes in.
