...
You and a coworker are sharing a todo list for a trip you're planning together. You're on a flight with no wifi. You add three tasks. They're at their desk, adding two more, at the same time. Neither of you can see what the other is doing. You land and reconnect. What happens?
...
This project is a working answer to that question: a todo list where two people can edit the same list while completely disconnected from each other. When they reconnect, everything merges back together automatically, deterministically, with nothing lost.
...
I built this back to back with [Bank Ledger](/case-studies/bank-ledger), a project that answers the same CAP-theorem question the opposite way. That one is a distributed ledger that refuses writes the instant it can't confirm consistency with quorum, because for money, a wrong number is worse than a rejected request. Building both sides of that decision, on the same kind of infrastructure, is what made the tradeoff concrete instead of theoretical for me: AP and CP aren't "better" and "worse," they're answers tuned to what a wrong answer costs. A todo list that briefly disagrees with itself is a minor annoyance. A ledger that does the same thing is an incident. I'm still working out how much of this is a general principle versus something I only really understood after building both sides myself. But the engineering skill isn't picking one side by default, it's recognizing which side a given system actually needs, and being able to build the mechanism for either one.
...
The first item on that list is a real bug, not a scope decision. I'd fix it before calling this "production-adjacent." Everything below it is a line I drew on purpose, so the project could stay legible as a demonstration of one specific idea rather than becoming a from-scratch Google Docs. If you'd have drawn any of these lines differently, I'd genuinely like to hear about it.
...
Full source, including the CRDT merge logic and sync handler, is in the [repo](https://github.com/riorhezaharris/todo-list).

If you clone it and find a case where the merge doesn't converge the way I think it does, or you'd have made a different call on any of the tradeoffs above, I'd like to hear about it. Better to find out from you than assume I got it all right.
